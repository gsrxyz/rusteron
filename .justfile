...
# Build the valgrind Docker image
_build-valgrind-image:
...
# Run tests under Valgrind inside Docker (builds first, then runs each test binary directly).
# Detects use-after-free, invalid reads/writes, and definite/indirect leaks.
# Suppresses known Aeron C and Rust runtime false-positives via valgrind.supp.
test-valgrind: _build-valgrind-image
    docker run --rm --platform=linux/amd64 \
      ...
      --entrypoint /bin/bash \
      rusteron-valgrind \
      /work/scripts/valgrind-run.sh

# Generate suppression entries for any errors Valgrind finds — append output to valgrind.supp.
# Usage: just valgrind-gen-suppressions >> valgrind.supp
valgrind-gen-suppressions: _build-valgrind-image
    docker run --rm --platform=linux/amd64 \
      --shm-size=2g \
      -e HOME=/tmp/valgrind \
      -e TMP=/tmp/valgrind/tmp \
